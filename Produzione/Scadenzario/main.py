...
def prepara_resoconto(tab, mode):
...
def trova_file():
    ...
    while found == 0:
        ...
        if server_dir[i][0:27] == "Scadenzario polizze_2024.09":
...
def main():
    ...
    try:
        ...
        t_left, t = prepara_resoconto(tab, mode)
        t_left = "Prossimo alert tra: "+str(round(dtLeft/3600/24)) + " giorni."

    except Exception as err:
        err_string = str(err)
        if err_string[:4] == "year":
            anno_sbagliato = err_string[5:11]
            text = "E' stato inserito l'anno anomalo " + anno_sbagliato + "\nProssimo alert tra 2 ore."
            segnalaErrore(mode, text)
            print(err)
            t = datetime.now() + timedelta(seconds=24*60*60)
            t_left = "Prossimo alert tra 24 ore"
        elif err_string[0:28]== "[Errno 13] Permission denied":
